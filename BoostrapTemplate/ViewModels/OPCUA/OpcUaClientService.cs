        ...
        if (_session != null)
        {
            if (_session.Connected)
            {
                try
                {
                    ...
                    {
                        var propertiess = item.GetType().GetProperties(); // Get all properties of the current item

                        foreach (var prop in propertiess)
                        {  
                            if (string.Equals(prop.Name, "Id", StringComparison.OrdinalIgnoreCase))
                            {
                                continue; // Skip this iteration for the 'Id' property
                            }
                           
                            var value = prop.GetValue(item)?.ToString();

                            if (!string.IsNullOrEmpty(value)) // Ensure the value is not null or empty
                            {
                    ...
                    var properties = _LineMgmtDetails.GetType().GetProperties();

                   
                    for (int i = 0; i < properties.Length-2 && i < v.Item1.Count; i++)
                    {
                        var property = properties[i]; 

                        var value = v.Item1[i]?.Value; 
                        if (value != null)
                        {
                           
                            var convertedValue = Convert.ChangeType(value, property.PropertyType);
                            property.SetValue(_LineMgmtDetails, convertedValue);
                        }
                       
                    }

                    #region Saperate Read And Bind
            ...
            if (_session.Connected)
            {
                try
                {
                    ...
                    foreach (var item in data)
                    {
                        ...
                        foreach (var prop in propertiess)
                        {  // Skip the 'Id' column (case-insensitive check)
                            if (string.Equals(prop.Name, "Id", StringComparison.OrdinalIgnoreCase))
                            {
                                continue; // Skip this iteration for the 'Id' property
                            }
                            ...
                            if (!string.IsNullOrEmpty(value)) // Ensure the value is not null or empty
                            {
                                // Create a NodeId using the property value
                                NodeId node = new NodeId(value);
                    ...
                    // Get all the properties of the class sequentially
                    var properties = _LineMgmtDetails.GetType().GetProperties();

                    // Iterate through the properties and bind values from v.Item1
                    for (int i = 0; i < properties.Length - 2 && i < v.Item1.Count; i++)
                    {
                        var property = properties[i]; // Get the property at index i

                        var value = v.Item1[i]?.Value; // Get the value from v.Item1
                        if (value != null)
                        {
